...
# palain, lyrane, (klovia), trenco
# _GOOGLEID = hashlib.md5(str(random.random()).encode('utf-8')).hexdigest()[:16]
# _COOKIES = {'GSP': 'ID={0}:CF=4'.format(_GOOGLEID)}

_COOKIES = {'GSP': 'LM=1533307165:S=h_3Oz35cb5OGMol0'}
...
def _get_page(pagerequest):
    ...
    waittime = 2+random.uniform(0, 2)
    print('Waiting for %d seconds' % waittime)
    time.sleep(waittime)
    resp = _SESSION.get(pagerequest, headers=_HEADERS, cookies=_COOKIES)
    print('%s - Got %s with status code: %s' % (time.asctime(time.localtime(time.time())), pagerequest, str(resp.status_code)))
    if resp.status_code == 200:
        if "Please show you&#39;re not a robot" in resp.text:
...
class Author(object):
    ...
    def fill(self, hindexonly=False):
